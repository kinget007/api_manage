...
<script type="text/javascript">
	...
	$(document).ready(function() {
		...
		Request = GetRequest();
		var categorySecondID = Request["categorySecondID"]; 
						...
						function() {
							...
							for(var j=0;j<eval(apiInfo).length;j++){
								if(apiInfo[j].apiID == apiID){
									...
									var attentionButton = "";
									if(code!=""){
										attentionButton = "<button type=\"button\" class=\"btn btn-Success\" id=\"attention\" name=\"attention\">关注</button>";
									}
									$("#mainContent").append(
				...
				$("#leftMenue").append(
						...
						"<div class=\"panel-heading\">"+
							"<a class=\"panel-title\" data-toggle=\"collapse\""+
								"data-parent=\"#panel-835660\" href=\"#panel-element-821428\">"+data[i].categorySecondName+"</a>"+
						"</div>"+
						"<div id=\"panel-element-821428\" class=\"panel-collapse in\">"+
							"<div class=\"panel-body\" id=\"apiListone\" style=\"height:600px;overflow:auto;\" >"+
	 ...
	 $(document).on('click', '#attention', function() {
		...
		$.getJSON(basePath+"/api_manage/addAttention?code="+code+"&apiID="+apiIDattention+"&result=add",null,function(data) {
			    if(data.result=="ok"){
				    $('#successMessage').text("关注成功");
					$('#successModal').modal('show');			    	
			    }
			    else{
			    	$('#successMessage').text("关注失败");
					$('#successModal').modal('show');
