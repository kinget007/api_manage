...
public class UrlRequest {
	...
	public static ArrayList<String> getHttpResponseContent(String urlPath,String paramInVauleJson,String httpHeader,String httpMethod) throws UnsupportedEncodingException{
		...
		for(int i =0;i<urlPath.split("\\$").length;i++){
			 ...
			 paramInURLs.add(urlPath.split("\\$")[i].subSequence(0, urlPath.split("\\$")[i].indexOf("}")).toString().trim());
			 }
		 }
		if(paramInVauleJson==null || paramInVauleJson.equals("")){
			resultInfo = httpclient_url(urlPath,tool.jsonTOHeadMap(httpHeader),httpMethod,jsonPost);
		}else{

			jsonParamALL = JSONArray.fromObject("[" + URLDecoder.decode(paramInVauleJson, "utf-8") + "]");

			String urlPath2 = URLDecoder.decode(urlPath, "utf-8");
			...
			for (int i = 0; i < jsonParamALL.size(); i++) {
				...
				while (iterator.hasNext()) {
					...
					if(key.contains("*")){
						if(key.contains("post")|| httpMethod.toLowerCase().contains("post")){
//							jsonPost.accumulate(key.substring(0, key.indexOf("*")),URLDecoder.decode(jsonObject.get(key).toString(), "utf-8") );
							jsonPost.accumulate(key.substring(0, key.indexOf("*")), jsonObject.get(key).toString());
						}else{
...
//								jsonURLPath.accumulate(key.substring(0, key.indexOf("*")), URLDecoder.decode(jsonObject.get(key).toString(), "utf-8"));
								jsonURLPath.accumulate(key.substring(0, key.indexOf("*")), jsonObject.get(key).toString());

							}else{
//								jsonURL.accumulate(key.substring(0, key.indexOf("*")), URLDecoder.decode(jsonObject.get(key).toString(), "utf-8"));
								jsonURL.accumulate(key.substring(0, key.indexOf("*")), jsonObject.get(key).toString());
...
//						jsonURL.accumulate(key, URLDecoder.decode(jsonObject.get(key).toString(), "utf-8"));
	...
	public static  ArrayList<String> httpclient_url(String url,HashMap<String, String> headers, String method,JSONObject jsonPost) throws UnsupportedEncodingException {
		System.out.println("url		"+url);
		System.out.println("url		" + jsonPost.toString());
		...
		for (Iterator iter = jsonPost.keys(); iter.hasNext();) {
			String key = (String)iter.next();
			params.add(new BasicNameValuePair(key,jsonPost.getString(key)));
		}
...
//				testHttpClient.testPost(params,url);

				HttpPost request = new HttpPost(url);
//				StringEntity entityPost = new StringEntity(jsonPost.toString(),"utf-8");//解决中文乱码问题
				HttpEntity entityPost = new UrlEncodedFormEntity(params, "UTF-8");
//				request.addHeader("Content-Type","application/json; charset=utf-8");
//				request.addHeader("Content-Type","application/json");
